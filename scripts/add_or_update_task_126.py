...
async def main():
    ...
    try:
        ...
        if existing_task:
            logger.info(f"📝 Задача 126 уже существует, обновляем...")
            logger.info(f"   Старое название: {existing_task.name}")
            logger.info(f"   Старый предмет: {existing_task.item_name}")
            ...
            if task:
                logger.info(f"✅ Задача 126 обновлена успешно!")
            else:
                logger.error(f"❌ Не удалось обновить задачу 126")
                return
        else:
            logger.info(f"➕ Задача 126 не найдена, создаем новую...")
            ...
            logger.info(f"   ID: {task.id} (будет другой, так как 126 уже занят)")
        ...
        logger.error(f"❌ Ошибка при создании/обновлении задачи: {e}")
        import traceback
