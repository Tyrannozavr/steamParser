"""
Скрипт для принудительного запуска парсинга задачи #118.
"""
import asyncio
import sys
...
async def run_task_118():
    """Принудительно запускает парсинг задачи #118."""
    # Инициализируем БД
    ...
    # Получаем задачу #118
    task = await db_session.get(MonitoringTask, 118)
    
    if not task:
        logger.error("❌ Задача #118 не найдена")
    ...
    await db_session.close()
    await db_manager.close()
    logger.info("✅ Задача отправлена, проверяйте логи в logs/tasks/task_118_*.log")


if __name__ == "__main__":
    asyncio.run(run_task_118())
