"""
Простой тест реального запроса к Steam Market API /render/ endpoint.
Использует только стандартную библиотеку и httpx.
"""
import asyncio
import json
import logging
...
async def test_real_api_request():
    ...
    try:
        ...
        data_str = str(data)
        logger.info(f"✅ Получен ответ от API (размер: {len(data_str)} символов)")
        
        # Извлекаем assets
        if 'assets' in data and '730' in data['assets']:
            app_assets = data['assets']['730']
                ...
                for itemid, item in items.items():
                    ...
                    if 'asset_properties' in item:
                        ...
                        for prop in props:
                            ...
                            logger.info(f"         propertyid={prop_id}, keys={list(prop.keys())}")
