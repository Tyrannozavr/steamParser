...
class RabbitMQService:
    ...
    async def connect(self):
        ...
        try:
            ...
            # Устанавливаем 10 минут (600000 мс) - это защита от зависших задач
            # После добавления таймаутов для БД операции не должны зависать дольше 30 секунд,
            # но на всякий случай оставляем 10 минут как защиту от других проблем
            self._parsing_queue = await self._channel.declare_queue(
                ...
                arguments={
                    ...
                    # Consumer timeout - автоматически возвращает неподтвержденные сообщения через 10 минут
                    # Это защита от зависших задач на уровне RabbitMQ
                    # После добавления таймаутов для БД операции завершаются за 10-30 секунд,
                    # поэтому 10 минут - это защита от других проблем (сеть, зависание воркера и т.д.)
                    "x-consumer-timeout": 10 * 60 * 1000,  # 10 минут в миллисекундах (600000 мс)
                }
            )
